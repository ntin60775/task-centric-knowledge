...
Реализация завершена. Все runtime-модули перенесены в `src/task_knowledge/`, импорты обновлены, тесты зелёные (260/260), verification matrix покрыта.
...
- Визуальная проверка структуры `src/task_knowledge/` — выполнена.
- Проверка глобальной установки — выполнена (`make install-global` и `make verify-global-install` проходят).
