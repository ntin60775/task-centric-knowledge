...
|---------|------------|------|------|------|----------------|--------|-----|--------------|---------|
| `DU-1` | `task/task-2026-0041-remove-legacy-facades` → `main` | `task/task-2026-0041-remove-legacy-facades` | `main` | `none` | `none` | `merged` | `—` | `1b9b62f` | `не требуется` |

## Текущий этап

Реализация завершена, финальный аудит пройден, все автоматические и ручные проверки выполнены. Задача готова к закрытию.
...
- Удалены три legacy-фасада (`scripts/install_skill.py`, `scripts/task_workflow.py`, `scripts/task_query.py`).
- Удалена legacy-ветка `--mode` из unified CLI.
- Обновлён `pyproject.toml`: очищен от facade-модулей.
- Все references (`adoption.md`, `deployment.md`, `task-workflow.md` и др.) переведены на `task-knowledge`.
- Обновлены `README.md`, `SKILL.md`, `Makefile`, managed-блоки.
- Тесты адаптированы: 260/260 зелёных.
- Verification matrix полностью покрыта.
- Delivery unit слит в `main` (commit `1b9b62f`).
