...
Задача переведена в работу по двум review-замечаниям: `task status` должен деградировать без `.git`, а `doctor` не должен смешивать project root consumer-а с source root standalone skill-а.
...
Реализация начата с review-замечаний 3.1 и 3.2. Первый fix-pass закрывает archive/zip деградацию `task status` без `.git` и делает `doctor` явным по границе project root consumer-а и standalone source root skill-а. Более широкий consumer contract по полному отказу от `.sisyphus` остаётся следующим этапом задачи.
