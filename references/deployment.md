...
После overlay helper запускает `make install-local` уже из live-copy, чтобы user-facing
`task-knowledge` был связан с установленной глобальной копией, а не только с source repo.
